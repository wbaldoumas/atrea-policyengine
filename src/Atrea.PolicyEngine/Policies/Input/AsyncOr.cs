...
/// <summary>
///     An <see cref="IAsyncInputPolicy{T}" /> which computes the boolean OR of two <see cref="IAsyncInputPolicy{T}" />s.
/// </summary>
/// <typeparam name="T">The type of the item being checked by this input policy.</typeparam>
public class AsyncOr<T> : IAsyncInputPolicy<T>
{
    private readonly IAsyncInputPolicy<T> _left;
    private readonly IAsyncInputPolicy<T> _right;

    /// <summary>
    ///     Construct an <see cref="AsyncOr{T}" /> input policy, which will compute the boolean OR of
    ...
    public AsyncOr(IAsyncInputPolicy<T> left, IAsyncInputPolicy<T> right)
    {
        _left = left;
        _right = right;
    }
